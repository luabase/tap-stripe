...
STREAM_SDK_OBJECTS = {
    ...
    "subscription_items": {
        "sdk_object": stripe.SubscriptionItem,
        "key_properties": ["id"],
    },
...
IMMUTABLE_STREAM_LOOKBACK = (
    600  # 10 min in epoch time, Stripe accuracy is to the second
)
...
def new_list(self, api_key=None, stripe_version=None, stripe_account=None, **params):
    ...
    try:
        ...
        if "No such invoice item" in str(error):
            ...
            LOGGER.warning(
                "%s. Currently, skipping this invoice line item call.", str(error)
            )
...
class Context:
    ...
    event_update_window_size = (
        DEFAULT_EVENT_UPDATE_DATE_WINDOW  # By default collect data of 7 days in one
    )
...
def configure_stripe_client():
    stripe.set_app_info(
        Context.config.get("user_agent", "Singer.io Tap"),
...
def load_shared_schema_refs():
    ...
    for shared_file in shared_file_names:
        with open(
            os.path.join(shared_schemas_path, shared_file), encoding="UTF-8"
        ) as data_file:
...
@backoff.on_exception(backoff.expo, stripe.error.RateLimitError, max_tries=7, factor=2)
...
def paginate(
    sdk_obj, filter_key, start_date, end_date, stream_name, request_args=None, limit=100
):
...
def get_bookmark_for_stream(stream_name, replication_key):
    ...
    if stream_name in ["invoices", "invoice_line_items"]:
        stream_bookmark = singer.get_bookmark(
            Context.state, stream_name, "date"
...
def write_bookmark_for_stream(stream_name, replication_key, stream_bookmark):
    ...
    # so kept `date` in bookmarking for Invoices and Invoice line Items as it has to respect bookmark of active connection too.
    if stream_name in ["invoices", "invoice_line_items"]:
        singer.write_bookmark(Context.state, stream_name, "date", stream_bookmark)
    else:
        singer.write_bookmark(
            Context.state, stream_name, replication_key, stream_bookmark
        )
    ...
    with Transformer(singer.UNIX_SECONDS_INTEGER_DATETIME_PARSING) as transformer:
        ...
        if stream_name in IMMUTABLE_STREAMS:
            if stream_name == "events":
                ...
                start_window = int(
                    max(bookmark, (singer.utils.now() - timedelta(days=30)).timestamp())
                )
                if start_window != bookmark:
                    LOGGER.warning(
                        "Provided start_date or current bookmark for newly created event records is older than 30 days."
                    )
                    LOGGER.warning(
                        "The Stripe Event API returns data for the last 30 days only. So, syncing event data from 30 days only."
                    )
            ...
            try:
                ...
                lookback_window = Context.config.get(
                    "lookback_window", IMMUTABLE_STREAM_LOOKBACK
                )
                if (
                    lookback_window
                    and int(lookback_window)
                    or lookback_window in (0, "0")
                ):
                ...
                raise ValueError(
                    "Please provide a valid integer value for the lookback_window parameter."
                ) from None
            if start_window != Context.config["start_date"]:
                start_window = evaluate_start_time_based_on_lookback(
        ...
        while start_window < end_time:
            ...
            if should_sync_sub_stream and stop_window > sub_stream_bookmark:
                ...
                write_bookmark_for_stream(
                    sub_stream_name, replication_key, sub_stream_bookmark
                )
...
def sync_sub_stream(sub_stream_name, parent_obj, updates=False):
    ...
    if sub_stream_name == "invoice_line_items":
        ...
        if "automatic" in parent_obj and parent_obj["automatic"]:
            ...
            LOGGER.info(
                "Skipping retrieval of balance history for manual payout %s", payout_id
            )
            return
    else:
        raise Exception(
    ...
    with Transformer(singer.UNIX_SECONDS_INTEGER_DATETIME_PARSING) as transformer:
        ...
        for sub_stream_obj in iterator:
            if expected_count:
                substream_count += 1
                if (
                    expected_count + INITIAL_SUB_STREAM_OBJECT_LIST_LENGTH
                ) < substream_count:
            ...
            if sub_stream_name == "invoice_line_items":
                ...
                if updates and obj_ad_dict.get("unique_id"):
                    ...
                    if obj_ad_dict.get("type") == "invoiceitem" and not obj_ad_dict.get(
                        "invoice_item"
                    ):
                        obj_ad_dict["invoice_item"] = object_id
                    # if type is subscription, update 'subscription' field with 'id' if not present
                    elif obj_ad_dict.get(
                        "type"
                    ) == "subscription" and not obj_ad_dict.get("subscription"):
            ...
            rec = transformer.transform(
                ...
                metadata.to_map(Context.get_catalog_entry(sub_stream_name)["metadata"]),
            )
...
def sync_event_updates(stream_name, is_sub_stream):
    ...
    events_update_date_window_size = int(
        60 * 60 * 24 * event_update_window_size
    ...
    parent_bookmark_value = (
        singer.get_bookmark(Context.state, stream_name + "_events", "updates_created")
        or start_date
    )

    # Get the bookmark value of the sub_stream if its selected and present
    if sub_stream_name:
        sub_stream_bookmark_value = (
            singer.get_bookmark(
                Context.state, sub_stream_name + "_events", "updates_created"
            )
            or start_date
        )
    ...
    if is_sub_stream:
        ...
        if (
            parent_bookmark_value != sub_stream_bookmark_value
            and bookmark_value == start_date
        ):
    ...
    max_event_start_date = (
        epoch_to_dt(sync_start_time) - timedelta(days=30)
    ).timestamp()
    max_created = int(max(bookmark_value, max_event_start_date))

    if max_created != bookmark_value and (
        bookmark_value != start_date or reset_brk_flag_value is True
    ):
        reset_bookmark_for_event_updates(
            is_sub_stream, stream_name, sub_stream_name, start_date
        )
        raise Exception(
            "Provided current bookmark date for event updates is older than 30 days."
            " Hence, resetting the bookmark date of respective {}/{} stream to start date.".format(
    ...
    while not stop_paging:
        ...
        if (
            not len(response) and date_window_end > extraction_time.timestamp()
        ...
        for events_obj in response.auto_paging_iter():
            ...
            with Transformer(
                ...
                if isinstance(events_obj.get("data").get("object"), stripe.Invoice):
                    ...
                    if line_items:
                        filtered_line_items = [
                            line_item for line_item in line_items if line_item.get("id")
                        ]
                    ...
                    if rec.get("id") is not None:
                        ...
                        if sub_stream_name and Context.is_selected(sub_stream_name):
                            if event_resource_obj:
                                sync_sub_stream(
                                    sub_stream_name, event_resource_obj, updates=True
                                )
        ...
        write_bookmark_for_event_updates(
            is_sub_stream, stream_name, sub_stream_name, max_created
        )
...
def write_bookmark_for_event_updates(
    is_sub_stream, stream_name, sub_stream_name, max_created
):
    """
    ...
    if not is_sub_stream:
        singer.write_bookmark(
            Context.state, stream_name + "_events", "updates_created", max_created
        )

    # Write the child bookmark value only when the child is selected
    if sub_stream_name and Context.is_selected(sub_stream_name):
        singer.write_bookmark(
            Context.state, sub_stream_name + "_events", "updates_created", max_created
        )

    singer.write_state(Context.state)


def reset_bookmark_for_event_updates(
    is_sub_stream, stream_name, sub_stream_name, start_date
):
    """
...
def sync():
    ...
    for catalog_entry in Context.catalog["streams"]:
        ...
        if Context.is_selected(stream_name):
            ...
            if not Context.is_sub_stream(stream_name) or not is_parent_selected(
                stream_name
            ):
...
def get_date_window_size(param, default_value):
    ...
    if (
        (type(window_size) in [int, float])
        or (isinstance(window_size, str) and window_size.replace(".", "", 1).isdigit())
    ) and float(window_size) > 0:
        return float(window_size)
    else:
        # Raise Exception if window_size value is 0, "0" or invalid string.
        raise Exception(
            "The entered window size '{}' is invalid, it should"
...
def main():
    ...
    # Otherwise run in sync mode
        ...
        Context.window_size = get_date_window_size(
            "date_window_size", DEFAULT_DATE_WINDOW_SIZE
        )
        Context.event_update_window_size = get_date_window_size(
            "event_date_window_size", DEFAULT_EVENT_UPDATE_DATE_WINDOW
        )
        ...
        if Context.event_update_window_size > 30:
            ...
            LOGGER.warning(
                "Using a default window size of 30 days as Stripe Event API returns data of the last 30 days only."
            )
